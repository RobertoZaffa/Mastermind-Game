...
class Mastermind_Engine:
    ...
    def left_codes(self, db_ac, ac, code_played, key_code ):         
        ...
        for i in range (ac):                                        #ripete per tutte le combinazioni di codici possibili
            ...
            if key_code == found_keycode and db_ac[i][4]==True:      #se il codice chiave trovato è uguale quello della giocata e il campo 4 è True ...
...
class Mastermind(Mastermind_Engine):
    ...
    def print_codes (self, lc, db_lc, bc, db_bc, elapsed_time):     #visualizza i left e i best codes
        print ("\nLeft Codes:", lc, " " * (13 - len(str(lc))), "Best Codes:", bc)
        for i in range (min(lc,20)):
            row_to_print = str(db_lc[i][0:4])+" "+ str(db_lc[i][13])
            if i < bc:
                row_to_print += " " * (14 - len(str(db_lc[i][13]))) + str(db_bc[i][0:4]) + " " + str(db_bc[i][13])
            print (row_to_print)                
...
while True: 
    print ("")
    print ("="*80)
    option = input ("1)Game1  2)Game2  3)Help  6)6 Colors(default)  8)8 Colors  9)End ")
    if option == "1":
